...
class BinaryPartialModeMetricBase(Observable, ABC):
    ...
    def _compute_metric_and_plot(
        ...
        with torch.no_grad():
            pl_module.model.binary("partial", update_binaries=self.update_binaries)
            ...
            if self.freq_idx[state] % self.freq[state] == 0:
                for metric_name in self.metrics:
                    ...
                    pl_module.log(f"binary_partial_mode/{metric_name}_{state}", metric)

                    trainer.logger.experiment.add_scalars(
                        f"comparative/binary_partial_mode/{metric_name}", {state: metric}, step
                    )

                    trainer.logger.log_metrics(
                        {f"binary_partial_mode/{metric_name}_{state}": metric}, step
                    )
                    trainer.logged_metrics.update(
                        {f"binary_partial_mode/{metric_name}_{state}": metric}
                    )
            ...
            if self.do_plot_update(state):
                ...
                trainer.logger.experiment.add_figure(f"preds/{state}/binary_partial_mode/input_pred_target", fig, step)
    ...
    def on_train_epoch_end(
        ...
        for metric_name in self.metrics.keys():
            ...
            trainer.logger.log_metrics(
                {f"binary_partial_mode/metrics_epoch_mean/{metric_name}_train": metric}, step=trainer.current_epoch
            )
            pl_module.log(f"binary_partial_mode/metrics_epoch_mean/per_batch_step/{metric_name}_train", metric)
    ...
    def on_validation_epoch_end(
        ...
        for metric_name in self.metrics.keys():
            ...
            trainer.logger.log_metrics(
                {f"binary_partial_mode/metrics_epoch_mean/{metric_name}_val": metric}, step=trainer.current_epoch
            )
            pl_module.log(f"binary_partial_mode/metrics_epoch_mean/per_batch_step/{metric_name}_val", metric)
    ...
    def on_test_epoch_end(
        ...
        for metric_name in self.metrics.keys():
            ...
            trainer.logger.log_metrics(
                {f"binary_partial_mode/metrics_epoch_mean/{metric_name}_test": metric}, step=trainer.current_epoch
            )
            pl_module.log(f"binary_partial_mode/metrics_epoch_mean/per_batch_step/{metric_name}_test", metric)
...
class BinaryPartialModeMetricMorpho(BinaryPartialModeMetricBase):
    def plot_step(self, state, inputs, preds, targets, pl_module):
        inputs, preds, targets = [k.cpu().detach().numpy()[0] for k in [inputs, preds, targets]]
        return PlotPreds.plot_three(
            inputs, preds, targets,
            title=f"binary_partial_mode/{state}",
            fig_kwargs={"vmin": 0, "vmax": 1}
        )


class BinaryPartialModeMetricClassif(BinaryPartialModeMetricBase):
...
class BinaryPartialModeMetricClassifChannel(BinaryPartialModeMetricBase):
