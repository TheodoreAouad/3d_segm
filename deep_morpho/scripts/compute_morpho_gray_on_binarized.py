...
def check_is_computed(tb_path):
    return os.path.exists(join(tb_path, "observables", "InvertGrayAndBinarizedMnist", "metrics.json"))
...
def run_test(tb_path):
    ...
    # we invert binarized mnist and grayscale mnist
    datasets_format = {
        "mnist": "mnistgrayscaledataset",
        "mnistmorpho": "mnistgrayscaledataset",
        "mnist_gray": "mnistmorphodataset",
        "mnistgraymorpho": "mnistmorphodataset",
    }

    if args["dataset_type"] not in datasets_format:
        return
    ...
    savepath = Path(tb_path) / "observables" / "InvertGrayAndBinarizedMnist" / "metrics.json"
